...
**主な内容（Step 1）**: Undo/Redo・ズーム/パン・消しゴム・スポイト・バケツ  
**主な内容（Step 2）**: PNG書き出し・ブラシサイズ数値入力・キャンバス回転  
**完了条件**: 1時間の描画セッションが不便なく進められ、成果物を PNG 保存できる
...
**目的**: フィルター・パスの前に、制作ワークフローに不可欠な基礎機能を揃える  
**主な内容**: 左右反転・ブラシカーソル・ショートカット・塗り許容値・背景色・自動保存・
ぼかし筆・直線・手ブレ補正UI・筆圧カーブ・選択範囲・移動・変形・カラーパレット・HSV・アルファロック  
**詳細**: `docs/plan/block-6.5-essentials.md`（Phase A〜E）  
**完了条件**: 一般的なイラスト制作のワークフローが一通り成立する

### Block 7: フィルター・パス（後フェーズ）
**目的**: SAI で不足していた機能領域を実装する  
**主な内容**: ぼかし/シャープ等フィルター・ベジェパス・テキスト  
**完了条件**: Block 6.5 完了後に詳細設計
