...
> 実装済み追加: Color EV スライダーの SDR/HDR 視覚分離UI（中央=EV0で区切り・右=発光トラック・初回ヒント）。
> 実装済み追加: Freeze（焼き込み）— 効果編集パネルに「効果+下を統合」「直下に焼込」（焼き込みは Undo 非対応）。
> 実装済み追加: スポイト知覚ギャップ色チップ（内部値 vs 表示。max(rgb)>1 OR viewEV≠0 OR tonemap≠none で表示）。
