...
describe('tool-config 整合性', () => {
  ...
  test('apply が EngineCtx の対応メソッドを正しく呼ぶ', () => {
    ...
    const mock: EngineCtx = {
      ...
      setPressureOpacity: (v) => calls.push(`pressureOpacity:${v}`),
  ...
  test('筆圧濃度はブラシと消しゴムに表示される', () => {
    assert.ok(getToolDef('brush').params.includes('pressureOpacity'));
    assert.ok(getToolDef('eraser').params.includes('pressureOpacity'));
    assert.strictEqual(PARAM_DEFS.pressureOpacity.kind, 'checkbox');
    assert.strictEqual(PARAM_DEFS.pressureOpacity.default, false);
  });
});
